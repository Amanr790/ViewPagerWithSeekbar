...
public class ComboSeekBar extends android.support.v7.widget.AppCompatSeekBar {
    ...
    public ComboSeekBar(Context context, AttributeSet attrs) {
        ...
        a.recycle();
        mThumb = new CustomThumbDrawable(mColor, mThumbRadius, ContextCompat.getDrawable(getContext(), R.drawable.ic_handle));
        setThumb(mThumb);
    ...
    public void setColor(int color) {
        mColor = color;
        mThumb.setColor(color,Utils.getBitmapFromDrawable(ContextCompat.getDrawable(getContext(),R.drawable.ic_handle)));
        setProgressDrawable(new CustomDrawable(this.getProgressDrawable(), this, mDotRadius, mThumb.getRadius(), mDots,
    ...
    public void setAdapter(List<String> dots) {
        mDots.clear();
        int index = 0;
        for (int i =0; i <=((dots.size()-1) * 10); i++) {
            Dot dot = new Dot();
            if ((i % 10) == 0) {
                dot.text = dots.get(i/10);
            } else {
                dot.text = "";
            }
            dot.id = index++;
